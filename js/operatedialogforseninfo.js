	    	        function ShowStructAdminDialog(SerialNumber) { 
	    	        	//window.location.href='ltl/structadmin.jsp?xh='+SerialNumber;
	    	        	//alert(SerialNumber)
	    	        	window.open("admin/ltl/structadmin.jsp?xh="+SerialNumber);
/*	    	        	windows.open('http://www.guet.edu.cn');
           var page = 'admin/ltl/structadmin.jsp?xh='+sn;
 ...
 */
	    	        ...
	    	        function ShowJSONDataPage(SerialNumber) { 
	    	        	//window.location.href='ltl/structadmin.jsp?xh='+SerialNumber;
	    	        	//alert(SerialNumber)
	    	        	window.open("jsondata.jsp?SerialNumber="+SerialNumber);
/*	    	        	windows.open('http://www.guet.edu.cn');
           var page = 'admin/ltl/structadmin.jsp?xh='+sn;
            art.dialog.open(page, {
                lock: true, //锁屏
...
function ShowCourseInforDialog(sn) {  
            var page = 'User/SensorInfoDetail.action?sn='+sn;
            art.dialog.open(page, {
                lock: true, //锁屏
                background: '#D8D8D8', // 背景色
                opacity: 0.87, // 透明度
	        ...
	        function ShowAddInforDialog() {  
	            //var page = 'admin/sensorinfoadd.jsp';
	        	var page = 'User/Sensorinfoadd.action';
	       ...
	       function ShowMultiDelDialog(sn){
	           ...
	           if(sns.length<=0)
	               ...
	               $.ajax(
	              {
	               url:"User/SensorInfoListMultiDel.action",
	   			dataType : "json", //返回的数据类型,text 或者 json数据，建议为json
	   			type : "post", //传参方式，get 或post
	   			data : {sn:sns.join(",") 
	   			 },
